									...
									__constant Camera* main_cam, int RR_THRESHOLD, int GI_CHECK, int reset, uint rand)
{
	...
	//If camera moves just write the current color, else we have to take average with the previous pass.
	if ( reset == 1 )
	{	
		color.w = 1;
...
{	
	...
	float4 total_color, throughput;	
	throughput = (float4) (1.f, 1.f, 1.f, 1.f);
	total_color = (float4) (0.f, 0.f, 0.f, 0.f);	
	
	
	if(GI_CHECK)
	{	
		int x = 0;	
		for(int i =0; i < RR_THRESHOLD; i++)
		//while(true)
		{
			...
			throughput = throughput * evaluateBRDF(new_ray.dir, -ray.dir, hit_info) * PI; // cos(theta)/PI - PDF for cos weighted sampling. cos get cancelled.
			
			// If it hits light source accumulate emission color.
			if(new_hitinfo.light_ID >= 0)
			{	
			   int j = new_hitinfo.light_ID;	   
			   total_color += throughput * light_sources[j].emissive_col * light_sources[j].power;	
				break;
			}		
...
{	
	...
	*seed = xor_shift(*seed);
	...
	for(int j = 0; j < LIGHT_SIZE; j++)
	{
	   ...
	   light_attenuation =  max(dot(-w_i, light_sources[j].norm), 0.0f) * 2.0 * light_sources[j].area / distance;
	   cosine_falloff = max(dot(w_i, hit_info.normal), 0.0f);	   
	   direct_color += evaluateBRDF(w_i, -ray.dir, hit_info) * light_sources[j].emissive_col * light_sources[j].power * cosine_falloff * light_attenuation;			
	}
	return direct_color;
...
int sampleLights(HitInfo hit_info, float* geometry_term, float* light_pdf, float4* w_i, uint* seed)
{
	...
	float gt_arr[LIGHT_SIZE]; // array of geometry terms for each light source
	float4 w_is[LIGHT_SIZE]; // Store every light direction
	int max_idx = -1;
	for(int i = 0; i < LIGHT_SIZE; i++)
	{
		...
		*seed = xor_shift(*seed);	
		...
		gt_arr[i] = light_sources[i].power * cosine_falloff / distance;
