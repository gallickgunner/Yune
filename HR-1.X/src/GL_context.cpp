...
namespace helion_render
{
    ...
    GL_context::GL_context()
    {
        ...
        options.samples_x = 2;
        options.samples_y = 2;
        options.gi_ray_bounces = 4;
        options.save_img_samples = 400;
        //ctor
    }
    ...
    void GL_context::createWindow(int window_width, int window_height, bool is_fullscreen)
    {
        ...
        if(is_fullscreen)
        {
            ...
            window = glfwCreateWindow(mode->width, mode->height, "Helion-Render", prim_monitor, NULL);
        ...
        else
        {
            ...
            window = glfwCreateWindow(window_width, window_height, "Helion-Render", NULL, NULL);
        }
    ...
    void GL_context::setupWidgets()
    {
        ...
        // Setup buttons on the panel.
        {
            ...
            // Setup widgets inside Popup panel
            {
                ...
                // Int box for Samples in X dimension.
                {

                    new nanogui::Label(popup, "Samples in X :", "sans-bold");
                    nanogui::IntBox<int>* int_box = new nanogui::IntBox<int>(popup, 2);
                    int_box->setEditable(true);
                    int_box->setSpinnable(true);
                    int_box->setFontSize(16);
                    int_box->setFixedSize(Eigen::Vector2i(100,20));
                    int_box->setValue(2);
                    int_box->setMinMaxValues(2, std::numeric_limits<int>::max());
                    int_box->setCallback([&](int samples_x){ this->options.samples_x = samples_x; });
                }
                // Int box for Samples in Y dimension.
                {
                    new nanogui::Label(popup, "Samples in Y :", "sans-bold");
                    nanogui::IntBox<int>* int_box = new nanogui::IntBox<int>(popup, 2);
                    int_box->setEditable(true);
                    int_box->setSpinnable(true);
                    int_box->setFontSize(16);
                    ...
                    int_box->setValue(2);
                    int_box->setMinMaxValues(2, std::numeric_limits<int>::max());
                    int_box->setCallback([&](int samples_y){ this->options.samples_y = samples_y; });
                }
                // Int box for Samples at which to save an image.
                {
                    new nanogui::Label(popup, "Save Image at Samples :", "sans-bold");
                    nanogui::IntBox<int>* int_box = new nanogui::IntBox<int>(popup, 400);
                    int_box->setEditable(true);
                    int_box->setSpinnable(true);
                    int_box->setFontSize(16);
                    int_box->setFixedSize(Eigen::Vector2i(100,20));
                    int_box->setValue(400);
                    int_box->setMinMaxValues(0, std::numeric_limits<int>::max());
                    int_box->setCallback([&](int x){ this->options.save_img_samples = x; });
                }
                // Int box for nuumber of GI ray bounces.
                {
                    new nanogui::Label(popup, "GI ray bounces :", "sans-bold");
                    nanogui::IntBox<int>* int_box = new nanogui::IntBox<int>(popup, 4);
                    int_box->setEditable(true);
                    int_box->setSpinnable(true);
                    int_box->setValue(4);
                    int_box->setFontSize(16);
                    ...
                    int_box->setCallback([&](int ray_bounces){ this->options.gi_ray_bounces = ray_bounces; });
                }
            }
        }
        ...
        //Setup text box to show actual kernel execution time.
